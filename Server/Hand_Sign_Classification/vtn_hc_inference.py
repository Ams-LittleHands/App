...
class VTNInferenceDataset():
    def __init__(self,video_url,label_url,
                pose_task_processor,pose_detector,pose_input_shape):
        self.frames, _, _ = torchvision.io.read_video(video_url,pts_unit='sec')
        self.label_url = label_url
        self.pose_task_processor = pose_task_processor
        self.pose_detector = pose_detector
        self.pose_input_shape = pose_input_shape
        self.__setup__()
    
    def __setup__(self):
        
        self.keypoints = self.detect_keypoints(self.frames)
    ...
    def get_start_and_end_position(self,l_center,r_center,LHips,RHips):
        ...
        for i in range(len(frame_status)-1):
            if frame_status[i+1] - frame_status[i] != 0:
                if not is_start:
                    start_idx.append(max(0,i-1)) # lùi lại 2 frame
                    is_start = True
                else:
                    is_start = False
                    end_idx.append(min(n_frame-1,i+2)) # tiến 2 frame
    ...
    def detect_keypoints(self,video_array): 
        keypoints  = []
        for frame  in video_array:
            with torch.no_grad():
                model_inputs, _ = self.pose_task_processor.create_input(frame.numpy(), self.pose_input_shape)
                pose_result = self.pose_detector.test_step(model_inputs)
                kp = pose_result[0].pred_instances['keypoints'].reshape(-1,2).tolist()
                prob = pose_result[0].pred_instances['keypoint_scores'].reshape(-1).tolist()
...
class Inference:
    ...
    def __setup__(self):
        ...
        self.sign_recognition_model = torch.jit.load("../Models/vtn_vn_sign.pt")
        self.sign_recognition_model.eval()
        
        
    def __setup_preprocess_model__(self):
        
        pose_deploy_cfg = 'mmdeploy/configs/mmpose/pose-detection_rtmo_onnxruntime_dynamic.py'
        pose_model_cfg = 'inference_config/rtmpose-m_8xb512-700e_body8-halpe26-256x192.py'
        pose_device = 'cpu'
        pose_backend_model = ['../Models/pose.onnx']
        
        
        # read deploy_cfg and model_cfg
        pose_deploy_cfg, pose_model_cfg = load_config(pose_deploy_cfg, pose_model_cfg)

        # build task and backend model
        self.pose_task_processor = build_task_processor(pose_model_cfg, pose_deploy_cfg, pose_device)
        self.pose_detector = self.pose_task_processor.build_backend_model(pose_backend_model)

        # process input image
        self.pose_input_shape = get_input_shape(pose_deploy_cfg)
    ...
    def predict(self,video_url):
        ...
        dataset = VTNInferenceDataset(video_url,'inference_config/200_sign_words.csv',
                                    self.pose_task_processor,self.pose_detector,self.pose_input_shape)
        predictions = []
...
if __name__ == "__main__":
    infer = Inference()
    sentence = infer.predict('/mnt/disk3/anhnct/Hand-Sign-Recognition/conversation/conversation_video/conversation.mp4')
    print(sentence)
